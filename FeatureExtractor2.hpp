#ifndef _FEATURE_EXTRACTOR2_HPP_
#define _FEATURE_EXTRACTOR2_HPP_
...
private:
	...
	cv::Mat mean_;
public:
	FeatureExtractor2(const std::string& protoFilePath, 
...
inline FeatureExtractor2::FeatureExtractor2(const std::string& protoFilePath, 
				const std::string& modelFilePath, 
				const std::string& meanFilePath,
				bool useGpu, int deviceId) {
	if (useGpu) {
...
inline std::vector<float> FeatureExtractor2::extractFeatures(cv::Mat img) {
	CHECK(img.type() == CV_8UC3);
	cv::resize(img, img, cv::Size(227, 227), cv::INTER_AREA);
...
inline void FeatureExtractor2::OpenCV2Blob(const std::vector<cv::Mat>& imgs) {
    if (imgs.empty()) {
...
inline void FeatureExtractor2::SetMean(const std::string& mean_file) {
	...
	* filled with this value. */
	...
	mean_ = cv::Mat(227, 227, mean.type(), channel_mean);
}

#endif //_FEATURE_EXTRACTOR2_HPP_